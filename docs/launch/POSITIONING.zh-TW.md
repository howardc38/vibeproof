...
## 2026-09-14：目前首頁與示範重點

本輪按「使用者痛點 → 實作能力 → 反向覆核限制」重排訊息。首頁先回答三件事：**真的能用？修好有證據？再次修改後，證據還有效？** 原來的刪測試、scope 與完整功能表保留在後面；它們解釋覆蓋面，不承擔第一眼的產品定位。

主示範沿用同一折扣案例，依序展示：錯誤結果與綠色測試並存 → 真正回歸測試的 red/green 與目標執行 → kernel 對同一成功紀錄判定 ANSWERED、改動後 STALE。新第三幕已納入可執行 demo 與 regression test。圖卡是結果的視覺重現，不是假造的 agent 對話或 terminal screenshot。

優先展示次序是普通使用者能看懂的三個證據瞬間；runtime 真相查詢及 maintenance 原 finding 收尾，適合後續專題。Checker 註冊的 bypass 驗證則面向會擴充規則的開發者。「Wow」是待使用者驗證的產品假設，不是獨家能力或採用率的證據。

覆核後保留的界線：STALE 不等於找到 regression；執行目標函式不證明所有斷言正確；需求交代不等於理解需求；多個 reviewer 不自動證明獨立判斷。完整採用仍有設定、測試運行及誤報處理成本，未有一般使用者淨省時的量化結果。

以下 2026-09-05 競品研究保留原取樣版本，不冒稱已重做市場比較。
