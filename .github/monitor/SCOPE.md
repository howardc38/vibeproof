...
## Tools
    ...
    allowed    Read · Glob · Grep; Bash only for approved commands below
    refused    Edit · Write
...
## Commands
    ...
    allowed    ./bin/v4 review add          raise a finding
               ...
               ./bin/v4 check               execute approved checks and record their results
...
Those records cover the normal workflow, not every direct filesystem access --
...
**Same model, same blind spots.** A host can launch a separate session, but the
kernel does not authenticate its context or independence. A separate session is
