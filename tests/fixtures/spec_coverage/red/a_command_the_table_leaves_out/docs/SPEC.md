...
skipping it as too short to be a mechanism. `v4 trend` is named right here, in a
sentence, which is what makes this case the one it is: the substring reading of
"is it documented" is satisfied and the table below is still short a row. The
`probe` checker is named so the other-direction rule is satisfied: probe.
